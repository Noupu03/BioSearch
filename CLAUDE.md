...
**Existing managers → `MonoRoutine`**: `InputManager`, `LogWindowManager`, `FileWindow`, `HybridCameraController`, `TimerManager`, `GameLoopManager`, `SelectPopupManager`, `GameSessionManager`, `SceneStartManager` were migrated from plain `MonoBehaviour` to `MonoRoutine` — `void Update()`/`void LateUpdate()` became `protected override void UpdateProcess()`/`LateUpdateProcess()` (each calling `base.UpdateProcess()`/`base.LateUpdateProcess()`), driven by `Processor` instead of Unity's native message. This is now every `MonoBehaviour` manager in the project. `SanityManager`/`GameStateManager`/`DummyIconSpawner` were candidates too but their features were deleted outright (Sanity system and E-key escape sequence dropped from the design) rather than migrated. `GameSessionManager` is `DontDestroyOnLoad`'d — that call moved into `Constructor()` along with the rest of its old `Awake()` body, same pattern as everything else.
