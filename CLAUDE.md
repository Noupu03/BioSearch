...
`GameLifetimeScope` (`Assets/Scripts/Haare/Client/Core/DI/GameLifetimeScope.cs`, global namespace) is a bare VContainer `LifetimeScope` with an empty `Configure()` — it's the DI entry point for new features that want constructor injection (`builder.Register<...>`, `builder.RegisterEntryPoint<...>`). It does **not** replicate HaareFramework's `CoreLifetimeScope` (no Addressables/DataManager/SceneService/UIPresenter coupling). **Tools → 씬 셋업** creates the `GameLifetimeScope` GameObject in the scene if missing (see `SetupGameLifetimeScope` in `SceneSetupTool.cs`) — it's idempotent, so re-running the tool won't duplicate it.

Not brought in (would require Addressables + a much larger surface area): `GamePresenter`/`IPresenter`/`UIPresenter` (VContainer Presenter layer), the stack-based `SceneUIManager`/`[PanelAttribute]` panel system, `DataManager`, `SceneService`, `AssetLoader`.
