...
**Existing managers → `MonoRoutine`**: `InputManager`, `LogWindowManager`, `FileWindow`, `HybridCameraController`, `TimerManager` were migrated from plain `MonoBehaviour` to `MonoRoutine` — `void Update()`/`void LateUpdate()` became `protected override void UpdateProcess()`/`LateUpdateProcess()` (each calling `base.UpdateProcess()`/`base.LateUpdateProcess()`), driven by `Processor` instead of Unity's native message. `SanityManager`/`GameStateManager`/`DummyIconSpawner` were candidates too but their features were deleted outright (Sanity system and E-key escape sequence dropped from the design) rather than migrated.

**`Awake()` pitfall (real bug hit and fixed)**: `MonoRoutine.Awake()` is `private` and does the async `Processor` registration. The first assumption was that a subclass defining its own `void Awake()` (for the `if (Instance != null) Destroy(gameObject); Instance = this;` singleton guard) would run *alongside* it, since Unity is commonly believed to invoke same-named lifecycle methods from every class in the hierarchy independently. **That assumption was wrong** — defining `Awake()` in the subclass silently prevented `MonoRoutine`'s own `Awake()` from running at all, so `UpdateProcess()` was never wired up (symptom: WASD input stopped working after converting `InputManager`). Fix: singleton-guard/init logic that lived in these managers' `Awake()` was moved into `protected override void Constructor()` instead — `MonoRoutine` calls `Constructor()` synchronously as the first line of its own `Awake()`-triggered init, so it's a safe, guaranteed hook (see `InputManager`, `LogWindowManager`, `FileWindow`). `OnDestroy()` in these classes was **not** touched despite being the same theoretical risk (their own `OnDestroy()` may be silently suppressing `MonoRoutine`'s own `OnDestroy()`, which unregisters the routine from `Processor`) — not yet confirmed broken in practice (the game scene isn't reloaded mid-run), but worth checking if `Processor`-related errors show up when leaving the game scene. `HybridCameraController`/`TimerManager` never defined their own `Awake()` so were unaffected by this.
