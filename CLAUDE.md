...
Found in passing, **not fixed** (pre-existing, unrelated to this migration): `Assets/Scripts/Camera/CameraSwitch.cs` duplicates a large chunk of `HybridCameraController`'s W/S camera-switch logic and is still attached to a GameObject in `scene ssh` — if both are active simultaneously they'd fight over the same camera transforms. `CameraSwitcher.cs`/`GameOverManager.cs` are separate, already-empty stubs explicitly marked dead by an earlier refactor (unrelated files, already unused).
